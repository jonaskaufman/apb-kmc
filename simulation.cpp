...
bool Simulation::is_valid_event(const Event& event) const
{
    if (boundary_type == BOUNDARY_TYPE::MINUS)
    {
...
bool Simulation::is_valid_event_zeta_minus(const Event& event) const
{
...
bool Simulation::is_valid_event_zeta_plus(const Event& event) const
{
...
double Simulation::calculate_barrier(const Event& event) const
{
    if (boundary_type == BOUNDARY_TYPE::MINUS)
    {
        return calculate_barrier_zeta_minus(event);
    }
    else
    {
        return calculate_barrier_zeta_plus(event);
    }
}
